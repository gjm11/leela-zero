...
First of all, a "fail" result need not mean that the new network is weaker. A network is promoted according to a criterion called SPRT (<https://en.wikipedia.org/wiki/Sequential_probability_ratio_test>), whose idea is to promote networks only when we're 95% sure that the net network will in the long run win at least 55% of games against the old network, so that if you switch to a new network you can be reasonably confident that LZ will genuinely get stronger as a result.
